...
CPB.add_halfspace!(domain, [1.0], -2)
A = reshape([-1.0], 1, 1)
b = [1.0]
CPB.add_piece!(sys, domain, 1, A, b, 2)

iset = InitSet()
CPB.add_state!(iset, State(1, [-1.0]))
CPB.add_state!(iset, State(1, [1.0]))

uset = UnsafeSet()
udom = Polyhedron()
CPB.add_halfspace!(udom, [-1.0], 1.1)
CPB.add_region!(uset, Region(2, udom))

lear = CPB.Learner(nvar, nloc, sys, iset, uset, 0.0, 0.0)
