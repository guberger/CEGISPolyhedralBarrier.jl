...
for nvar in (4, 5, 6, 7, 8, 9)
    ...
    domain1 = CPB.Cone()
    CPB.add_supp!(domain1, -a)
    A1 = U \ (ONE_ - (nvar + 1)*EYE_) * U
    domain2 = CPB.Cone()
    CPB.add_supp!(domain2, a)
    ...
    for γ in (1.0, 0.1, 0.01)
        sys = CPB.System()
        CPB.add_piece_cont!(sys, domain1, 1, A1)
        A2 = U \ (ONE_ - (nvar + γ)*EYE_) * U
        CPB.add_piece_cont!(sys, domain2, 1, A2)
        for δ in (γ/50, γ)
            global iter += 1
            iter > max_iter && break
            lear = CPB.Learner(nvar, nloc, sys, τ, ϵ, δ)
            status, mpf, niter = @time CPB.learn_lyapunov!(
                lear, 1000, solver, solver, do_print=false
            )
            time = @elapsed status2, mpf, niter2 = CPB.learn_lyapunov!(
                lear, 1000, solver, solver, do_print=false
            )
            @assert niter == niter2
            @assert status == status2
            @assert status ∈ (CPB.LYAPUNOV_FOUND, CPB.LYAPUNOV_INFEASIBLE)
